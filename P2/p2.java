 ...
 * Sample run on Windows Terminal: javac *.java; java -cp ";./db2jcc4.jar" p2 ./db.properties;
 */

 public class p2 extends BankingSystem {
  ...
  private static String[][] menus = {
    ...
    "4.  Add Interest to All Active Accounts",
    "5.  Exit"}
  };
  ...
  public static void displayMenu() {
    if(!loggedInCustomerID.equals(nonLoggedInCustomerID)) System.out.println("Currently Logged in as ID: " + loggedInCustomerID);
        ...
        switch(menuState) {
          ...
          case ADMINMENU: { // Administrator Main Menu
            switch(inputInt) {
              ...
              case 4: { // Add Interest
                String savingsRate, checkingRate;
                String firstPrompt = "Please enter the Savings Rate (As a float, ex: 0.05): ";
                System.out.print(firstPrompt);
                savingsRate = scan.nextLine().trim();
                System.out.format("%" + firstPrompt.length() + "s", "Checking Rate: ");
                checkingRate = scan.nextLine().trim();
                bankingSystem.addInterest(savingsRate, checkingRate);
                break;
              }
              case 5: { 
                logout();
